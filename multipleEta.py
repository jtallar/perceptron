...
while j < len(etas):
    ...
    training_set, expected_out_set, test_training_set, test_expected_out_set \
        = parser.extract_subset(full_training_set, full_expected_out_set, test_ratio, 0)
    ...
    if plot_bool and not cross_validation:
        ...
        eta_list.append(eta)
        acc_train_superlist.append(acc_train_total)
        acc_test_superlist.append(acc_test_total)
...
if plot_bool and len(eta_list) > 1:
    metrics.plot_multiple_values([it for i in range(len(acc_train_superlist))], "Iterations", acc_train_superlist, "Accuracy Train", eta_list, min_val=0, max_val=100)
    metrics.plot_multiple_values([it for i in range(len(acc_test_superlist))], "Iterations", acc_test_superlist, "Accuracy Test", eta_list, min_val=0, max_val=100)
